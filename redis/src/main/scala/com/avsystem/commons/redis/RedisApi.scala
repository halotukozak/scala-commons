 ...
 *
 * `Async` and `Blocking` API variants additionally come in three different "levels", each one exposing different subset
 * of Redis commands. This reflects the fact that not every [[RedisExecutor]] (client implementation) supports every
 * command (e.g. you can't execute unkeyed commands using [[RedisClusterClient]]).
 *
 ...
 *
 * Every API variant may also use different types to represent Redis keys, hash keys and values. You can define your own
 * API variants for arbitrary combination of key, hash key and value types as long as there is an instance of
 ...
 *
 * API variants which use only `String`s (textual) or only `ByteString`s (binary) are already implemented by the driver,
 * e.g. [[RedisApi.Keyed.Async.StringTyped]], [[RedisApi.Batches.BinaryTyped]].
 *
 ...
 * Redis.
 *
 * Note that chosen key, hash key and value types can be adjusted "on the fly" with a convenient syntax. For example, if
 * you need to use some case class as a value type in a single, specific place, you can do it without defining a
...
object RedisApi {
  class Raw[S <: RedisSerialization](val serialization: S)
      extends AbstractRedisApi[S]
      with RedisConnectionApi
      with RedisRawApi {
  ...
  object Keyed extends ExecutedApis {
    ...
    case class Async[S <: RedisSerialization](serialization: S, executor: RequiredExecutor, execConfig: ExecutionConfig)
        extends BaseAsync[S]
        with RedisRecoverableKeyedApi
    object Async extends VariantCompanion[Async]

    case class Monix[S <: RedisSerialization](serialization: S, executor: RequiredExecutor, execConfig: ExecutionConfig)
        extends BaseMonix[S]
        with RedisRecoverableKeyedApi
...
abstract class ExecutedApis {
  ...
  abstract class VariantCompanion[Variant[S <: RedisSerialization] <: AbstractRedisApi[S]] {
    ...
    def StringTyped(exec: RequiredExecutor, config: ExecutionConfig = ExecutionConfig.Default): StringTyped =
      apply(RedisSerialization.Strings, exec, config)

    type BinaryTyped = Variant[RedisSerialization.ByteStrings.type]

    def BinaryTyped(exec: RequiredExecutor, config: ExecutionConfig = ExecutionConfig.Default): BinaryTyped =
      apply(RedisSerialization.ByteStrings, exec, config)
  }
