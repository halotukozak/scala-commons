 ...
 * Execution of a batch may also fail for various reasons. Therefore, [[RedisBatch]] contains API that allows to recover
 * from failures, e.g. [[RedisBatch.tried]].
 *
...
trait RedisBatch[+A] { self =>
  ...
  def atomic: RedisBatch[A] =
    if rawCommandPacks.computeSize(2) > 1 then transaction else this
...
object RedisBatch extends HasFlatMap[RedisBatch] {
  ...
  def traverse[M[X] <: IterableOnce[X], A, B, That](
    coll: M[A],
  )(opFun: A => RedisBatch[B])(implicit bf: BuildFrom[M[A], B, That]): RedisBatch[That] = {
  ...
  def foldLeftMap[A, B, T](coll: IterableOnce[A], zero: T)(
    opFun: A => RedisBatch[B],
  )(fun: (T, B) => T): RedisBatch[T] = {
    val batches = new ArrayBuffer[RedisBatch[B]]
    coll.iterator.foreach(a => batches += opFun(a))
 ...
 * result of some previous operation (therefore, `flatMap` is available). [[RedisOp]] is guaranteed to be executed fully
 * and exclusively on a single Redis connection (no other concurrent commands can be executed on that connection during
 * execution of [[RedisOp]]). Because of that, [[RedisOp]]s may execute `WATCH` and `UNWATCH` commands.
 *
