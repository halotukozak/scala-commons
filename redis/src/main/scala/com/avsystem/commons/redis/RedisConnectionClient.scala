 ...
 * [[config.ConnectionConfig ConnectionConfig]] (which may also be specified for connections used by [[RedisNodeClient]]
...
final class RedisConnectionClient(
  ...
  def executeOp[A](op: RedisOp[A], executionConfig: ExecutionConfig): Future[A] =
    ifReady(
      system
        .actorOf(Props(new RedisOperationActor(connectionActor)))
        .ask(op)(executionConfig.responseTimeout)
