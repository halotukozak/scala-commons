...
object ReplyDecoders {
  ...
  val positiveIntegerAsLongOpt: ReplyDecoder[Opt[Long]] = { case IntegerMsg(value) =>
    if value > 0 then value.opt else Opt.Empty
  }
  ...
  def flatMultiBulkAsPairSeq[A, B](
    firstDecoder: ReplyDecoder[A],
    secondDecoder: ReplyDecoder[B],
  ...
  def flatMultiBulkAsSwappedPairSeq[A, B](
    firstDecoder: ReplyDecoder[A],
    secondDecoder: ReplyDecoder[B],
  ...
  def geoAttributed[A](
    attributes: GeoradiusAttrs,
    unattributed: ReplyDecoder[A],
  ): ReplyDecoder[attributes.Attributed[A]] =
    if attributes.isEmpty then unattributed.andThen(attributes.decode(ArrayMsg.Empty, attributes.flags, _))
    else {
