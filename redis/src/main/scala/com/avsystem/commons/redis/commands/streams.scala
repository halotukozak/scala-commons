...
trait StreamsApi extends ApiSubset {
  ...
  def xclaim(
    ...
    execute(
      new Xclaim(key, group, consumer, minIdleMillis, ids, idleMillis.toOpt, msUnixTime.toOpt, retrycount.toOpt, force)
    )
  ...
  private final class Xadd(key: Key, maxlen: Opt[XMaxlen], id: Opt[XEntryId], data: Record)
    extends AbstractRedisCommand[XEntryId](bulkAsXEntryId)
    with NodeCommand {
    val encoded: Encoded = encoder("XADD")
      .key(key)
  ...
  private final class XgroupCreate(key: Key, group: XGroup, id: Opt[XEntryId], mkstream: Boolean)
    ...
    with NodeCommand {
    val encoded: Encoded = encoder("XGROUP", "CREATE")
      .key(key)
      .add(group)
      .optAdd(id, "$")
      .addFlag("MKSTREAM", mkstream)
      .result
  }
  ...
  private final class Xpending(key: Key, group: XGroup)
    extends AbstractRedisCommand[XPendingOverview](multiBulkAsXPendingOverview)
    with NodeCommand {
  ...
  private final class XpendingEntries(
    ...
    with NodeCommand {
    val encoded: Encoded = encoder("XPENDING")
      .key(key)
      .add(group)
      .optAdd(start, "-")
      .optAdd(end, "+")
  ...
  private final class Xrange(key: Key, start: Opt[XEntryId], end: Opt[XEntryId], count: Opt[Int])
    extends RedisSeqCommand[XEntry](multiBulkAsXEntryOf[Record])
    with NodeCommand {
    val encoded: Encoded = encoder("XRANGE")
      .key(key)
      .optAdd(start, "-")
      .optAdd(end, "+")
      .optAdd("COUNT", count)
  ...
  private abstract class AbstractXread(noStreams: Boolean)
    extends AbstractRedisCommand[BMap[Key, Seq[XEntry]]](multiBulkAsXEntriesMapOf[Key, Record])
    with NodeCommand {
  ...
  private final class Xrevrange(key: Key, end: Opt[XEntryId], start: Opt[XEntryId], count: Opt[Int])
    extends RedisSeqCommand[XEntry](multiBulkAsXEntryOf[Record])
    with NodeCommand {
    val encoded: Encoded = encoder("XREVRANGE")
      .key(key)
      .optAdd(end, "+")
      .optAdd(start, "-")
      .optAdd("COUNT", count)
...
object XEntryId {
  ...
  given ordering: Ordering[XEntryId] = _ compare _

  given commandArg: CommandArg[XEntryId] =
...
object XMaxlen {
  given commandArg: CommandArg[XMaxlen] = CommandArg { case (enc, XMaxlen(maxlen, approx)) =>
...
object XGroup {
  given commandArg: CommandArg[XGroup] = CommandArg((e, v) => e.add(v.raw))
}

final case class XConsumer(raw: String) extends AnyVal
object XConsumer {
  given commandArg: CommandArg[XConsumer] = CommandArg((e, v) => e.add(v.raw))
}

case class XPendingOverview(count: Long, minId: XEntryId, maxId: XEntryId, countByConsumer: BMap[XConsumer, Long])
object XPendingOverview {
  final val Empty = XPendingOverview(0, XEntryId.Zero, XEntryId.Zero, Map.empty)
}

case class XPendingEntry(id: XEntryId, consumer: XConsumer, idleTime: Long, deliveredCount: Int)
