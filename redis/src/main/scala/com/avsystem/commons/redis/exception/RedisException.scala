 ...
 */
class ErrorReplyException(val reply: ErrorMsg, val command: RedisCommand[?])
 ...
 * connected to one of the master nodes in Redis Cluster and
 * [[com.avsystem.commons.redis.RedisClusterClient RedisClusterClient]] detected that this node is no longer a master
 * before the command could be sent to it.
 */
...
class TooManyRedirectionsException(val lastRedirection: Redirection)
    extends RedisException(
      s"Too many Redis cluster redirections, last one to: " +
        s"${lastRedirection.address} (slot ${lastRedirection.slot}${if lastRedirection.ask then ", ASK" else ""})",
    )
