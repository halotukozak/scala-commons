...
trait SortedSetsApi extends ApiSubset {
  ...
  def zrandmember(key: Key, count: Int, distinct: Boolean = true): Result[Seq[Value]] =
    execute(new ZrandmemberCount(key, if distinct then count else -count))

  /** Executes [[http://redis.io/commands/zrandmember ZRANDMEMBER]] */
  def zrandmemberWithscores(key: Key, count: Int, distinct: Boolean = true): Result[Seq[(Value, Double)]] =
    execute(new ZrandmemberWithscores(key, if distinct then count else -count))
  ...
  def zscan(
    key: Key,
    cursor: Cursor,
    matchPattern: OptArg[Value] = OptArg.Empty,
  ...
  private final class Zadd(
    ...
    override def immediateResult: Opt[Int] = if emptyData then Opt(0) else Opt.Empty
  }
  ...
  private final class Zinter(keys: Iterable[Key], weights: Opt[Iterable[Double]], aggregation: Opt[Aggregation])
      extends RedisDataSeqCommand[Value]
      with NodeCommand {
  ...
  private final class ZinterWithscores(
    keys: Iterable[Key],
    weights: Opt[Iterable[Double]],
    aggregation: Opt[Aggregation],
  ) extends AbstractValuesWithScoresCommand
      with NodeCommand {
    val encoded: Encoded = encoder("ZINTER")
      .add(keys.size)
      .keys(keys)
  ...
  private final class Zmscore(key: Key, members: Iterable[Value])
      extends RedisSeqOptCommand[Double](bulkAsDouble)
      with NodeCommand {
  ...
  private final class Zscan(key: Key, cursor: Cursor, matchPattern: Opt[Value], count: Opt[Int])
      extends RedisScanCommand[(Value, Double)](flatMultiBulkAsPairSeq(bulkAs[Value], bulkAsDouble))
      with NodeCommand {
    val encoded: Encoded =
  ...
  private final class ZunionWithscores(
    keys: Iterable[Key],
    weights: Opt[Iterable[Double]],
    aggregation: Opt[Aggregation],
  ) extends AbstractValuesWithScoresCommand
  ...
  private final class Bzpopmax(keys: Iterable[Key], timeout: Int)
      extends AbstractRedisCommand[Opt[(Key, Value, Double)]](multiBulkAsZTripleOf[Key, Value])
      with NodeCommand {
    val encoded: Encoded = encoder("BZPOPMAX").keys(keys).add(timeout).result

    override def immediateResult: Opt[Opt[(Key, Value, Double)]] =
      if keys.isEmpty then Opt(Opt.Empty) else Opt.Empty
...
case class ScoreLimit(value: Double, inclusive: Boolean) {
  def repr: String = (if !inclusive then "(" else "") + (value match {
...
object LexLimit {
  ...
  def repr[V: RedisDataCodec](limit: LexLimit[V]): ByteString = limit match {
    case Finite(value, incl) =>
      ByteString((if incl then '[' else '(').toByte) ++ RedisDataCodec.write(value)
