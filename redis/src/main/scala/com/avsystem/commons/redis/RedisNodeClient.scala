...
final class RedisNodeClient(
  ...
  private def ifReady[T](code: => Future[T]): Future[T] =
    failure.fold(if initSuccess then code else overallInitFuture.flatMapNow(_ => code))(Future.failed)
  ...
  private def executeOp[A](connection: ActorRef, op: RedisOp[A])(implicit timeout: Timeout): Future[A] =
    system
      .actorOf(Props(new RedisOperationActor(connection)))
      .ask(op)
      .mapNow({ case or: OpResult[A @unchecked] => or.get })

  /**
   * Waits until all Redis connections are initialized and `initOp` is executed. Note that you can call [[executeBatch]]
   * and [[executeOp]] even if the client is not yet initialized - requests will be internally queued and executed after
