 ...
 * type. For example, if `Ops` is `(RedisBatch[Int], RedisBatch[String])` (tuple) then `Res` will be `(Int, String)`. If
 * `Ops` is `List[RedisBatch[Int&#93;]` then `Res` will be `List[Int]`.
 *
...
object Sequencer extends TupleSequencers {
  ...
  implicit def collectionSequencer[ElOps, ElRes, M[X] <: IterableOnce[X], That](implicit
    ...
    (ops: M[ElOps]) => {
      val batches: Iterable[RedisBatch[ElRes]] =
        if (elSequencer eq reusableTrivialSequencer) && ops.isInstanceOf[Iterable[Any]] then
...
final class CollectionBatch[A, C](batches: Iterable[RedisBatch[A]], builderCreator: () => mutable.Builder[A, C])
    extends RedisBatch[C]
    with RawCommandPacks {
  ...
  def computeSize(limit: Int): Int =
    if limit <= 0 then limit else batches.foldLeft(0)((s, b) => s + b.rawCommandPacks.computeSize(limit - s))
