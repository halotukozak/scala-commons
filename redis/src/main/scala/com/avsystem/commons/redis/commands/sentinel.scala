...
trait SentinelApi extends ApiSubset {
  ...
  private object SentinelMasters
      extends AbstractRedisCommand(multiBulkAsSeq(flatMultiBulkAsMapOf[String, String]))
      with NodeCommand {
    val encoded: Encoded = encoder("SENTINEL", "masters").result
  }

  private final class SentinelMaster(masterName: String)
      extends AbstractRedisCommand(flatMultiBulkAsMapOf[String, String])
      with NodeCommand {
  ...
  private final class SentinelGetMasterAddrByName(masterName: String)
      extends AbstractRedisCommand(multiBulkAsNodeAddress)
      with NodeCommand {
