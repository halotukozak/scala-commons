...
object ConnectionSetupExample extends App {
  ...
  val authConfig = ConnectionConfig(initCommands = RedisApi.Batches.StringTyped.auth("mypassword"))
  ...
  val authSelectConfig = ConnectionConfig(initCommands = api.auth("mypassword") *> api.select(2))
  ...
  val nodeClient = new RedisNodeClient(config = NodeConfig(connectionConfigs = _ => authConfig))
  ...
  val clusterClient = new RedisClusterClient(config =
    ClusterConfig(
