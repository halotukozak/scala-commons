    ...
    redisClient.initialized.flatMapNow { client =>
      ...
      if sourceClient != targetClient then {
        ...
        val keysToMigrateFut =
          if withoutData then Future.successful(Seq.empty)
          else
            for {
        ...
        for {
          ...
          _ <-
            if keys.nonEmpty then sourceApi.migrate(keys, targetClient.address, 0, Int.MaxValue)
            else Future.successful(())
          _ <- {
            if incomplete then Future.successful(())
