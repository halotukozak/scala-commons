...
trait UsesRedisNodeClient extends UsesRedisServer with UsesActorSystem with UsesSslContext { this: Suite =>
  ...
  def connectionConfig: ConnectionConfig =
    ConnectionConfig(sslEngineCreator = if useTls then OptArg(() => sslContext.createSSLEngine()) else OptArg.Empty)

  def nodeConfig: NodeConfig =
    NodeConfig(connectionConfigs = _ => connectionConfig, blockingConnectionConfigs = _ => connectionConfig)

  var redisClient: RedisNodeClient = _

  override protected def beforeAll(): Unit = {
    super.beforeAll()
    redisClient = new RedisNodeClient(if useTls then tlsAddress else address, nodeConfig)
  }
