...
trait MacroCommons {
  ...
  //  final lazy val SeqCompanionSym = TypeRepr.of[scala.collection.Seq.type].termSymbol
  ...
  //  final lazy val StringPFTpe: Type = TypeRepr.of[PartialFunction[String, Any]]
  //  final lazy val BIterableTpe: Type = TypeRepr.of[Iterable[Any]]
  //  final lazy val BIndexedSeqTpe: Type = TypeRepr.of[IndexedSeq[Any]]
  //  final lazy val ProductTpe: Type = TypeRepr.of[Product]
  //  final lazy val AnnotationTpe: Type = TypeRepr.of[scala.annotation.Annotation]
  //  final lazy val StaticAnnotationTpe: Type = TypeRepr.of[scala.annotation.StaticAnnotation]
  ...
  //
  //    Simplified representation of trees of implicits
  //  , used to remove duplicated implicits
  //  ,
  //  i.e.implicits that were found
  //  for different types but turned out to be identical
  //    If
  //  implicit traces
  //  for two found implicits are identical
  //  then they are guaranteed to evaluate
  //  to the same runtime value.Therefore we can reuse the same
  //  implicit for different types by casting it.
  ...
  //  if this is a `val` constructor parameter
  //  , include `val` itself and its
  //  super symbols
  ...
  //            if lo =:= TypeRepr.of[Nothing] && bases.size >= 2 && bases.exists(_ =:= TypeRepr.of[Singleton]) =>
  //
  //            val strName = sym.name.decodedName.toString.stripSuffix(".type")
  //            val newBases = bases.filterNot(_ =:= TypeRepr.of[Singleton])
  ...
  //        if tree.tpe <:< TypeRepr.of[PartialFunction[Nothing, Any]] =>
  //        Some(cases)
  //      case _ => None
  //    }
  //  }
  ...
  //          sig => isParameterless(sig) && sig.finalResultType =:= TypeRepr.of[Boolean])
  //
