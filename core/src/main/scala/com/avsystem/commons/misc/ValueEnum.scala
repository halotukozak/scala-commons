 ...
 * as instances of the enum class in `final val` fields. This is an alternative way of implementing enums as compared to
 * traditional Scala approach of using a sealed hierarchy with objects representing enum values.
 *
 ...
 * [[SealedEnumCompanion]] and [[NamedEnumCompanion]]</li> <li>It is possible to define all enum values in a single line
 * of code (assuming they don't take parameters)</li> </ul> Disadvantages of value based enums over object based enums
 * include: <ul> <li>Every object can have its own separate public API, values cannot (although you can have a separate
 * anonymous class for every value)</li> <li>Scala compiler does not perform exhaustive checking for pattern matching
 * enum values - this is however provided separately by `commons-analyzer` compiler plugin.</li> </ul>
 *
 ...
 * ordinal and name as well as makes sure that enum value is registered in the companion object. If possible, you should
 * always extend [[AbstractValueEnum]] instead of mixing in this trait. [[ValueEnum]] trait should only be mixed in
 * directly when your enum class already has another superclass, incompatible with [[AbstractValueEnum]].
 *
 * @example
 *   {{{
 ...
 *   }}}
 */
trait ValueEnum(using protected val enumCtx: EnumCtx) extends NamedEnum {
 ...
 */
abstract class AbstractValueEnum(using enumCtx: EnumCtx) extends ValueEnum
...
trait ValueEnumCompanion[T <: ValueEnum] extends NamedEnumCompanion[T] { companion =>
  ...
  final def values: IndexedSeq[T] = synchronized {
    if _values == null then {
      ...
      finished = true
      _values = registryBuilder.result()
    }
    _values
  }

  implicit def ordering: Ordering[T] = Ordering.by(_.ordinal)

  implicit def ordered: Conversion[T, Ordered[T]] = Ordered.orderingToOrdered
  ...
  private class Ctx(val valName: String, val ordinal: Int) extends EnumCtx {
    ...
    override def register(value: ValueEnum): Unit = companion.synchronized {
      ...
      else if registered then throw new IllegalStateException("Cannot register using the same EnumCtx more than once")
      else {
  ...
  protected inline given valName: ValName = enumValName(new ValName(_))

  protected implicit def enumCtx(using valName: ValName): EnumCtx = new Ctx(valName.valName, currentOrdinal)
}

/**
 * Convenience abstract class implementing [[ValueEnumCompanion]]. For less generated code, faster compilation and
 * better binary compatibility it's better to use this abstract class rather than [[ValueEnumCompanion]] trait directly.
...
abstract class AbstractValueEnumCompanion[T <: ValueEnum]
    extends AbstractNamedEnumCompanion[T]
    with ValueEnumCompanion[T]
