...
final class CharSubSequence private (private val seq: CharSequence, private val start: Int, private val end: Int)
    extends CharSequence {
  def length: Int = end - start

  def charAt(index: Int): Char =
    if index >= 0 && index < length then seq.charAt(start + index)
    else throw new IndexOutOfBoundsException(index.toString)

  def subSequence(subStart: Int, subEnd: Int): CharSequence =
    if subStart < 0 || subEnd > length || subStart > subEnd then throw new IllegalArgumentException
...
object CharSubSequence:
  ...
  def apply(seq: CharSequence, start: Int, end: Int): CharSequence =
    if start < 0 || end > seq.length || start > end then throw new IllegalArgumentException
    else if start == 0 && end == seq.length then seq
