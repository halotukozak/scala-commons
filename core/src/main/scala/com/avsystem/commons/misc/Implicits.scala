...
object Implicits {
   ...
   * givenue (without being wrapped as implicit parameter of a dummy method like `implicitly`). Also, using `infer` lets
   * you have more detailed control over implicit-not-found compilation error messages through [[ImplicitNotFound]].
   */
  def infer[T]: T = ???

  //  macro macros.misc.MiscMacros.infer[T]
  def infer[T](clue: String): T = ???

  //  macro macros.misc.MiscMacros.clueInfer[T]
  def inferNonMacro[T](clue: String): T = ???
  //  macro macros.misc.MiscMacros.inferNonMacro[T]
}

/**
 * Extends the functionality of [[scala.annotation.implicitNotFound]] so that implicit-not-found error message is itself
 * resolved using implicit search. This mechanism is used by [[Implicits.infer[T]:T*]] and macro engines that use it.
 *
 ...
 *   trait Box[T]
 *   object Box {
 *     given boxCodec[T: GenCodec]: GenCodec[Box[T]] = ...
 *
 *     @implicitNotFound("GenCodec for Box[$${T}] not found. This is most likely caused by lack of GenCodec[$${T}]")
 *     given boxCodecNotFound[T]: ImplicitNotFound[GenCodec[Box[T]]] = ImplicitNotFound()
 *   }
 * }}}
 *
...
object ImplicitNotFound {
  ...
  given [T]: ImplicitNotFound[T] = apply()
}
