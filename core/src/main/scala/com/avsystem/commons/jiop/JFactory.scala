...
import collection.{CrossBuilder, CrossFactory}
...
trait JIterableFac extends JSetFac with JLinkedListFac:
  ...
  given jArrayListFac[A]: JCollectionFactory[A, JArrayList[A]] = new JCollectionFactory(new JArrayList)

trait JSetFac extends JSortedSetFac with JLinkedHashSetFac:
  // for JSet and JHashSet
  given jHashSetFac[A]: JCollectionFactory[A, JHashSet[A]] = new JCollectionFactory(new JHashSet)

trait JLinkedHashSetFac:
  given jLinkedHashSetFac[A]: JCollectionFactory[A, JLinkedHashSet[A]] =
