...
final case class RawCbor(bytes: Array[Byte], offset: Int, length: Int) {
  ...
  def apply(idx: Int): Byte =
    if idx < 0 || idx >= length then throw new IndexOutOfBoundsException
  ...
  def compact: RawCbor =
    if offset == 0 && length == bytes.length then this
    else safeCopy
}
object RawCbor extends TypeMarker[RawCbor] {
  ...
  given codec: GenCodec[RawCbor] =
    GenCodec.nonNull(
      ...
      (output, cbor) =>
        if !output.writeCustom(RawCbor, cbor) then {
