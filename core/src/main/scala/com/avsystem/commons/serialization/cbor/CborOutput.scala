...
import serialization.GenCodec.WriteFailure
import serialization.cbor.InitialByte.IndefiniteLength
...
sealed abstract class BaseCborOutput(out: DataOutput) {
  ...
  private def unsignedInfo(unsignedBytes: Long): Int =
    if unsignedBytes >= 0 && unsignedBytes < InitialByte.SingleByteValueInfo then unsignedBytes.toInt
    else if (unsignedBytes & 0xffL) == unsignedBytes then InitialByte.SingleByteValueInfo
    else if (unsignedBytes & 0xffffL) == unsignedBytes then InitialByte.TwoBytesValueInfo
    else if (unsignedBytes & 0xffffffffL) == unsignedBytes then InitialByte.FourBytesValueInfo
  ...
  protected def writeSigned(value: Long): Unit =
    if value >= 0 then writeValue(MajorType.Unsigned, value)
...
class CborOutput(out: DataOutput, keyCodec: CborKeyCodec, sizePolicy: SizePolicy)
  ...
  def writeBoolean(boolean: Boolean): Unit =
    write(if boolean then InitialByte.True else InitialByte.False)
  ...
  def writeDouble(double: Double): Unit =
    if double.toLong.toDouble == double then {
      ...
      if float.toDouble == double then {
        val hfloat = HFloat.fromFloat(float)
        if hfloat.toFloat == float then {
          write(InitialByte.HalfPrecisionFloat)
  ...
  def writeBigInt(bigInt: BigInt): Unit = {
    val neg = bigInt < 0
    val unsigned = if neg then -(bigInt + 1) else bigInt
    if unsigned.bitLength <= 64 then {
      writeValue(if neg then MajorType.Negative else MajorType.Unsigned, unsigned.longValue)
    } else {
      writeTag(if neg then Tag.NegativeBignum else Tag.PositiveBignum)
  ...
  override def writeTimestamp(millis: Long): Unit = {
    ...
    if millis % 1000 == 0 then writeLong(millis / 1000)
    else writeDouble(millis.toDouble / 1000)
  }
...
sealed abstract class CborSequentialOutput(out: DataOutput, override val sizePolicy: SizePolicy)
    extends BaseCborOutput(out)
  ...
  protected final def ensureInitialWritten(major: MajorType): Unit =
    if fresh then {
      ...
      } else if sizePolicy != SizePolicy.Required then {
...
class CborListOutput(out: DataOutput, keyCodec: CborKeyCodec, sizePolicy: SizePolicy)
...
class CborObjectOutput(out: DataOutput, keyCodec: CborKeyCodec, sizePolicy: SizePolicy)
    extends CborSequentialOutput(out, sizePolicy)
    with ObjectOutput {

  private var forcedKeyCodec: CborKeyCodec = compiletime.uninitialized

  private def currentKeyCodec = if forcedKeyCodec != null then forcedKeyCodec else keyCodec
   ...
   * [[writeKey]]. This method MUST ONLY be used after the key has been fully written with [[writeKey]]. If [[writeKey]]
   * and [[writeValue]] is used then [[writeField]] MUST NOT be used.
   */
