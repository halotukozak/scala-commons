...
trait CborOptimizedCodecs {
  ...
  private def mkMapCodec[M[X, Y] <: AnyRef, K: GenCodec: OptGenKeyCodec, V: GenCodec](
  ...
  )(using Factory[(K, V), M[K, V]]): GenObjectCodec[M[K, V]] = {
    ...
    new GenObjectCodec[M[K, V]] {
      override def readObject(input: ObjectInput): M[K, V] =
        if input.customEvent(ForceCborKeyCodec, cborKeyCodec) then hexKeysStdCodec.readObject(input)
        else regularStdCodec.readObject(input)

      override def writeObject(output: ObjectOutput, value: M[K, V]): Unit =
        if output.customEvent(ForceCborKeyCodec, cborKeyCodec) then hexKeysStdCodec.writeObject(output, value)
        else regularStdCodec.writeObject(output, value)
    }
  }
}
