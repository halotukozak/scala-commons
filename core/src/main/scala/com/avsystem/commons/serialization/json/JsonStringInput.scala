...
import misc.CrossUtils
import serialization.*
import serialization.GenCodec.ReadFailure
import serialization.json.JsonStringInput.*
...
class JsonStringInput(
  ...
  private var endIdx: Int = uninitialized
  ...
  override def readShort(): Short = parseNumber { str =>
    if isInteger(str) then str.toShort
    else {
      val dbl = str.toDouble
      if dbl.isValidShort then dbl.toShort
      else throw new NumberFormatException(str)
    }
  }

  def readInt(): Int = parseNumber { str =>
    if isInteger(str) then str.toInt
    else {
      val dbl = str.toDouble
      if dbl.isValidInt then dbl.toInt
      else throw new NumberFormatException(str)
    }
  }
  ...
  def readBigInt(): BigInt = parseNumber { str =>
    if isInteger(str) then BigInt(str)
    else {
      val bd = BigDecimal(str, options.mathContext)
      if bd.isWhole then bd.toBigInt
      else throw new NumberFormatException(str)
    }
  }
  ...
  private def checkedValue[T](jsonType: JsonType): T =
    if reader.jsonType != jsonType then expectedError(jsonType)
    else reader.currentValue.asInstanceOf[T]

  def readLong(): Long = parseNumber { str =>
    if isInteger(str) then str.toLong
    else {
      val bd = BigDecimal(str, options.mathContext)
      if bd.isValidLong then bd.toLong
      else throw new NumberFormatException(str)
    }
  }
  ...
  private def isInteger(str: String): Boolean = {
    ...
    str.nonEmpty && {
      if str.charAt(0) == '-' then str.length > 1 && loop(1)
      else loop(0)
    }
  }
  ...
  def readBinary(): Array[Byte] = options.binaryFormat match {
    ...
    case JsonBinaryFormat.HexString =>
      ...
      @tailrec def loop(i: Int): Unit = if i < result.length then {
  ...
  override def readByte(): Byte = parseNumber { str =>
    if isInteger(str) then str.toByte
    else {
      val dbl = str.toDouble
      if dbl.isValidByte then dbl.toByte
      else throw new NumberFormatException(str)
    }
  }
...
final class JsonListInput(reader: JsonReader, options: JsonOptions, callback: AfterElement)
  ...
  private def prepareForNext(first: Boolean): Unit = {
    ...
    if end then {
      reader.advance()
      callback.afterElement()
    } else if !first then reader.pass(',')
  }
}
...
final class JsonObjectInput(reader: JsonReader, options: JsonOptions, callback: AfterElement)
  ...
  private var peekIdx = if end then -1 else reader.index
  private var peekedFields: CrossUtils.NativeDict[Int] = uninitialized
        ...
        @tailrec def peekRemainingFields(): Opt[FieldInput] =
          if peekIdx < 0 then Opt.Empty
            ...
            if foundName == name then {
              ...
              if peekedFields == null then peekedFields = CrossUtils.newNativeDict[Int]
...
final class JsonReader(val json: String) {
  ...
  private var tpe: JsonType = uninitialized
  ...
  def posInfo(offset: Int): String = {
    @tailrec def loop(idx: Int, line: Int, column: Int): String =
      if idx >= offset then {
        ...
        val lineEnd = json.indexOf('\n', idx) |> (i => if i == -1 then json.length else i)
  ...
  def skipWs(): Unit =
    while i < json.length && Character.isWhitespace(json.charAt(i)) do {
      i += 1
    }

  def pass(ch: Char): Unit = {
    val r = read()
    if r != ch then readFailure(s"'${ch.toChar}' expected, got ${if r == -1 then "EOF" else r.toChar}")
  }

  def fromHex(ch: Char): Int =
    if ch >= 'A' && ch <= 'F' then ch - 'A' + 10
    else if ch >= 'a' && ch <= 'f' then ch - 'a' + 10
    else if ch >= '0' && ch <= '9' then ch - '0'
  ...
  private def currentCharOrEOF =
    if i < json.length then json.charAt(i).toString else "EOF"
