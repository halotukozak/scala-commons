...
import serialization.macros.whenAbsentValueImpl

/**
 * An alternative way to provide default value for case class parameter used during deserialization with `GenCodec` when
 * its field is missing in data being deserialized. Normally, Scala-level default parameter values are picked up, but
 * you may want to use this annotation instead if you don't want to pollute your Scala classes with unintended default
 * parameter values (i.e. you want a default value *only* for deserialization).
 *
 * {{{
 ...
 */
final class whenAbsent[+T](v: => T) extends StaticAnnotation {
  def value: T = v
}
...
object whenAbsent:
   ...
   * }}}
   *
   * This is useful when you want the default value to be collectible by macros (e.g. `OpenApiMetadata` for REST). which
   * is possible only with default value in annotation.
   */
  inline def value[T]: T = ${ whenAbsentValueImpl[T] }
