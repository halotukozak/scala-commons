 ...
 *
 * Such format is often useful as an intermediate representation. For example, it can be later safely passed to standard
 * Java serialization. However, for performance reasons it's recommended to implement dedicated [[Input]] and [[Output]]
 * for the final format (e.g. binary or JSON).
 *
 * @param consumer
...
class SimpleValueInput(value: Any) extends InputAndSimpleInput {
  ...
  def readObject(): ObjectInput =
    new ObjectInput {
      ...
      override def knownSize: Int = if map.isEmpty then 0 else map.knownSize

      override def nextField(): SimpleValueFieldInput = it.next()

      override def peekField(name: String): Opt[SimpleValueFieldInput] =
        map.getOpt(name).map(new SimpleValueFieldInput(name, _)) // values may be null!
  ...
  def readList(): ListInput =
    new ListInput {
      ...
      override def knownSize: Int = if inputSeq.isEmpty then 0 else inputSeq.knownSize
