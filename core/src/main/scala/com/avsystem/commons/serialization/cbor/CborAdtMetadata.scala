...
import annotation.{AnnotationAggregate, positioned}
import meta.*
import misc.ValueOf
import serialization.*
import serialization.GenCodec.OOOFieldsObjectCodec
 ...
 * hierarchies with annotations: [[cborKey]] and [[cborDiscriminator]], again taking advantage of the fact that CBOR map
 * keys can be of arbitrary type and not just strings</li> </ul>
 */
...
/**
 * Apply this annotation on a sealed trait/class whose companion extends [[HasCborCodec]] in order to customize the CBOR
 * field key used for discriminator field. Note: this annotation automatically applies [[flatten]] annotation on the
 * sealed trait/class.
 */
...
object CborAdtMetadata extends AdtMetadataCompanion[CborAdtMetadata] {
        ...
        val codecWithAdjustedCaseCodecs =
          new FlatSealedHierarchyCodec[T](
            ...
            override protected def doWriteCaseName(output: Output, caseName: String): Unit =
              if !output.writeCustom(RawCbor, caseNamesKeyCodec.rawKeys(caseName)) then {
  ...
  @positioned(positioned.here)
  final class Record[T](@composite val keyInfo: CborKeyInfo[T], @multi @adtParamMetadata val fields: List[Field[?]])
      extends Case[T] {
    ...
    def ensureUniqueKeys(discriminatorKey: Opt[RawCbor]): Unit = {
      for disc <- discriminatorKey; field <- fields do {
        if disc == field.keyInfo.rawKey then {
          throw new Exception(
  ...
  final class Field[T](@composite val keyInfo: CborKeyInfo[T]) extends TypedMetadata[T]

  @positioned(positioned.here)
  final class Singleton[T](@infer @checked val value: ValueOf[T], @composite val keyInfo: CborKeyInfo[T])
      extends Case[T] {
  ...
  private class DiscriminatorCborKeyCodec(strKey: String, rawDisc: RawCbor) extends CborKeyCodec {
    def writeFieldKey(fieldName: String, output: CborOutput): Unit =
      if fieldName == strKey then output.writeRawCbor(rawDisc)
      else output.writeString(fieldName)

    def readFieldKey(input: CborInput): String = {
      val rawKey = input.readRawCbor()
      if rawKey == rawDisc then strKey else rawKey.readAs[String]()
    }
  }
}
 ...
 */
abstract class HasPolyCborCodec[C[_]](using instances: MacroInstances[CborOptimizedCodecs, CborAdtPolyInstances[C]]) {
