...
//    given lazyMetadata[Real](implicit metadata: M[Real]): Lazy[Real] = macro MiscMacros.lazyMetadata
//
//    @implicitNotFound("#{forNotLazy}")
//    given notFound[T](implicit forNotLazy: ImplicitNotFound[M[T]]): ImplicitNotFound[Lazy[T]] =
//      ImplicitNotFound()
//  }
//}
//
...
//
//    // macro effectively turns `metadata` param into by-name param (implicit params by themselves cannot be by-name)
////    given lazyMetadata[Real >: Lo <: Hi](implicit metadata: M[Real]): Lazy[Real] = macro MiscMacros.lazyMetadata
//
//    @implicitNotFound("#{forNotLazy}")
//    given notFound[T >: Lo <: Hi](implicit forNotLazy: ImplicitNotFound[M[T]]): ImplicitNotFound[Lazy[T]] =
//      ImplicitNotFound()
//  }
//}
