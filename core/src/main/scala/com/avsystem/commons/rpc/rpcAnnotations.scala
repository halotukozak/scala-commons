 ...
 * `_<idx>` appended, where `<idx>` is an index of the overload, starting from 1 for the first actual overload. The
 * first method is not considered an "overload" and will get no suffix appended.
 *
 ...
 * should pass real method's RPC name as this parameter and that macro generated implementation of `AsRaw` should expect
 * real method's RPC name to be passed there.
 *
...
/**
 * Base trait for [[verbatim]] and [[encoded]]. These annotations can be applied either on a raw method or raw parameter
 * in order to specify how matching real method results or matching real parameter values are encoded as raw values.
 * Currently there are two possible cases: [[verbatim]] (no encoding) and [[encoded]] (encoding using `AsRaw` and
 * `AsReal` typeclasses). By default, method return values and [[com.avsystem.commons.meta.multi multi]] parameters are
 * [[encoded]] while [[com.avsystem.commons.meta.single single]] and [[com.avsystem.commons.meta.optional optional]]
 * parameters are [[verbatim]]. See documentation of [[verbatim]] and [[encoded]] for more details.
 */
 ...
 * strings that has appropriate implicit instances of `AsRaw` and `AsReal` (or `AsRawReal` which serves as both `AsReal`
 * and `AsRaw`).
 *
 * {{{
 ...
 *
 *   given genCodecBasedJsonEncoding[T: GenCodec]: AsRawReal[T,Json] =
 ...
 *   given genCodecBasedFutureJsonEncoding[T: GenCodec]: AsRawReal[Future[Json],Future[T]] =
 ...
 * do it by moving implicit instances of `AsReal` and `AsRaw` (or the joined `AsRawReal`) into the `implicits` object of
 * raw RPC companion:
 *
 * {{{
 * trait AsyncRawRpc {
 ...
 *     given genCodecBasedJsonEncoding[T: GenCodec]: AsRawReal[String,T] =
 *       AsRawReal.create[String,T](writeJson[T], readJson[T])
 *     given genCodecBasedFutureJsonEncoding[T: GenCodec]: AsRawReal[Future[String],Future[T]] =
...
/**
 * Turns off raw value encoding as specified by [[encoded]]. By default, [[com.avsystem.commons.meta.single single]] and
 * [[com.avsystem.commons.meta.optional optional]] raw parameters are already [[verbatim]], so using [[verbatim]] only
 * makes sense on [[com.avsystem.commons.meta.multi multi]] raw parameters or raw methods themselves, which means
 ...
 * annotation may then specify a `NewRaw` type. Macro engine will then look for a different implicit for deserialization
 * \- instead of looking for `AsReal[Raw,Real]` it will now look for `AsReal[NewRaw,Real]`. The annotation must
 * therefore provide a conversion from the original raw type to the new raw type.
 *
 ...
 * }
 * object ExampleApi {
 *   given AsRawReal: AsRawReal[ExampleRawRpc,ExampleApi] = AsRawReal.materialize
 * }
 * }}}
 *
 * @tparam BaseTag
...
/**
 * Parameter tagging lets you have more explicit control over which raw parameters can match which real parameters. This
 * way you can have some of the parameters annotated in order to treat them differently, e.g. they may be [[verbatim]],
 * encoded in a different way or collected to a different raw container (e.g. `Map[String,Raw]` vs `List[Raw]` - see
 * [[com.avsystem.commons.meta.multi multi]] for more details).
 *
 * Example:
 * {{{
