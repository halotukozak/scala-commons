...
object TypeStringTest {
  ...
  def defineTests[T: TypeString](suite: TypeStringTest): Unit = {
    ...
    testTypeString[List[T]](s"List[T]")
    testTypeString[TypeStringTest#B]("B")
    testTypeString[x.type]("x.type")
    ...
    // Scala 3 doesn't support type projections
    ...
    testTypeString[ {def lol(): Int}]("{def lol(): Int}")
    // Scala 3 doesn't support polymorphic refinements
    ...
    testTypeString[ {def lol(using x: Int, y: String): String}]("{def lol(using x: Int, y: String): String}")
    testTypeString[ {def lol(x: String): x.type}]("{def lol(x: String): x.type}")
    testTypeString[ {type T; def lol: T}]("{type T; def lol: T}")
  }
}

final class TypeStringTest extends AnyFunSuite:

  inline def testTypeString[T: TypeString](expected: String)(using pos: Position): Unit =
    test(s"${pos.lineNumber}:$expected") {
      assert(TypeString.of[T].replace("com.avsystem.commons.misc.", "") == expected)
    }
  ...
  testTypeString[fuu.bar.q.type]("fuu.bar.q.type")
  testTypeString[None.type]("None.type")
  testTypeString[this.type]("TypeStringTest.this.type")
  testTypeString[List[Int]]("List[Int]")
  testTypeString[Set[?]]("Set[?]")
  testTypeString[Set[? <: String]]("Set[? <: String]")
  testTypeString[AnyRef & Serializable]("AnyRef & Serializable")
