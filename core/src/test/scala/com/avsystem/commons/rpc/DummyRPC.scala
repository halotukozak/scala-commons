...
//
//  given anyAsRawReal[T: GenCodec]: AsRawReal[String, T] =
//    AsRawReal.create(write[T], read[T])
//
//  given readerBasedFutureAsReal[T: GenCodec]: AsReal[Future[String], Future[T]] = _.mapNow(read[T])
//  given writerBasedFutureAsRaw[T: GenCodec]: AsRaw[Future[String], Future[T]] = _.mapNow(write[T])
//
//  given codecFromTag[T](implicit tag: Tag[T]): GenCodec[T] = tag.codec
//
//  trait Instances[T] {
