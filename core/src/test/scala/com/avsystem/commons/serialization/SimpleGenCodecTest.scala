...
import misc.TypedMap
import serialization.JavaCodecs.given
...
final class SimpleGenCodecRoundtripTest extends GenCodecRoundtripTest with SimpleIOCodecTest

@nowarn
final class SimpleGenCodecTest extends SimpleIOCodecTest {
  ...
  final case class Node[T](value: T, children: List[Node[T]] = Nil)

  object Node extends HasPolyGenCodec[Node]

  test("recursive generic case class") {
    testWrite(
      Node(123, List(Node(42, List(Node(52), Node(53))), Node(43))),
      Map[String, Any](
        "value" -> 123,
        "children" -> List(
          Map[String, Any](
    ...
    testWrite[Tree[Int]](
      ...
      Map(
        "Branch" -> Map(
          ...
          "right" -> Map(
            "Branch" -> Map("left" -> Map("Leaf" -> Map("value" -> 2)), "right" -> Map("Leaf" -> Map("value" -> 3))),
          ),
        ),
      ),
    )
  ...
  test("refined sealed type with type member") {
    testWrite[SealedRefined { type X = Int }](SealedRefined.First(42), Map("First" -> Map("foo" -> 42)))
  }
