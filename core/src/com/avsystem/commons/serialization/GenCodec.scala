...
object GenCodec extends RecursiveAutoCodecs with TupleGenCodecs {
  ...
  implicit def seqCodec[C[X] <: BSeq[X], T: GenCodec](implicit fac: Factory[T, C[T]]): GenCodec[C[T] & BSeq[T]] =
    nullableList[C[T] & BSeq[T]](_.collectTo[T, C[T]], (lo, c) => c.writeToList(lo))

  implicit def setCodec[C[X] <: BSet[X], T: GenCodec](implicit fac: Factory[T, C[T]]): GenCodec[C[T] & BSet[T]] =
    nullableList[C[T] & BSet[T]](_.collectTo[T, C[T]], (lo, c) => c.writeToList(lo))

  implicit def jCollectionCodec[C[X] <: JCollection[X], T: GenCodec](implicit
    cbf: JFactory[T, C[T]]
