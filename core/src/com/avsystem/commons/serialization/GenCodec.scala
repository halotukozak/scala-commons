...
object GenCodec extends RecursiveAutoCodecs with TupleGenCodecs {
  ...
  given seqCodec[C[X] <: BSeq[X], T: GenCodec](using fac: Factory[T, C[T]]): GenCodec[C[T] & BSeq[T]] =
    nullableList[C[T] & BSeq[T]](_.collectTo[T, C[T]], (lo, c) => c.writeToList(lo))

  given setCodec[C[X] <: BSet[X], T: GenCodec](using fac: Factory[T, C[T]]): GenCodec[C[T] & BSet[T]] =
