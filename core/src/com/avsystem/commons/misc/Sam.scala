...
import com.avsystem.commons.macros.newAnon
...
object Sam {
  ...
  /** Implements a single abstract method trait/class `T` using passed function or expression as implementation of the
    ...
    */
  inline def apply[T](fun: => Any): T = ${ applyImpl[T]('{ fun }) }

  def applyImpl[T: Type](value: Expr[Any])(using quotes: Quotes): Expr[T] = {
    import quotes.reflect.*
    value match
      case '{ $value: f } =>
        val method = TypeRepr.of[T].typeSymbol.methodMembers.find(_.flags.is(Flags.Deferred)).get
        ...
        def defn(rhsFn: List[List[Tree]]) = Some {
          val byName = rhsFn == List(Nil) && TypeRepr.of[f] <:< returnTpe
        ...
        '{
          compiletime.summonInline[ValidSam[T, f]]
          $anonDef
        }
  }
