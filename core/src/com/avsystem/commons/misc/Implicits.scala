...
object Implicits {
  ...
  /** Similar to `implicitly` from standard library but implemented as a macro which materializes directly into the
    * given value (without being wrapped as implicit parameter of a dummy method like `implicitly`). Also, using `infer`
    * lets you have more detailed control over implicit-not-found compilation error messages through
    * [[ImplicitNotFound]].
    */
