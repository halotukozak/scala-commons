...
trait Components extends ComponentsLowPrio {
  ...
  /** Creates a [[Component]] based on a definition (i.e. a constructor invocation). The definition may refer to other
    ...
    */
  protected def component[T](definition: ComponentInfo ?=> T)(implicit sourceInfo: SourceInfo): Component[T] =
    ??? // macro ComponentMacros.component[T]

  /** Asynchronous version of [[component]] macro.
    */
  protected def asyncComponent[T](definition: ExecutionContext => Future[T])(implicit
  ...
  /** Asynchronous version of [[singleton]] macro.
    */
  protected def asyncSingleton[T](definition: ExecutionContext => Future[T])(implicit
    sourceInfo: SourceInfo
  ...
  // i.e. the compiler will emit "could not find given value" instead of "divergent implicit expansion"
