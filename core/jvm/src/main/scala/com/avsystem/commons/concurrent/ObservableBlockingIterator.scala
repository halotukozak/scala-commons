...
final class ObservableBlockingIterator[T](observable: Observable[T], timeout: Long, unit: TimeUnit, bufferSize: Int)(
  ...
  @volatile private var ackPromise: Promise[Ack] = uninitialized
  private val queue = new ArrayBlockingQueue[Any](bufferSize)
  private val cancelable = observable.subscribe(this)

  def onNext(elem: T): Future[Ack] = {
    val safeElem = if elem.asInstanceOf[AnyRef] eq null then Null else elem
  ...
  private def fetchNext(): Any = last match
    case Empty =>
      ...
      if promise != null && queue.remainingCapacity >= bufferSize / 2 then {
