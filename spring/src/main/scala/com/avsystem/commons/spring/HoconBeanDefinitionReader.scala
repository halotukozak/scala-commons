...
class HoconBeanDefinitionReader(registry: BeanDefinitionRegistry) extends AbstractBeanDefinitionReader(registry) {
  ...
  private def validateObj(
    ...
    iterate(obj) { (key, value) =>
      if !allAllowed.contains(key) then badAttr(key, value)
    } { (key, value) =>
      if !props then badProp(key, value)
    }
  }
  ...
  private object BeanDefinition {
    ...
    def unapply(obj: ConfigObject) =
      if BeanOnlyAttrs.exists(obj.as[ConfigObject].keySet.contains) then Some(obj) else None
  }

  private class ObjectWithAttributePresentExtractor(elementAttr: String) {
    def unapply(obj: ConfigObject) =
      if obj.containsKey(elementAttr) then Some(obj) else None
  }
  ...
  private def readBean(obj: ConfigObject) = {
    ...
    obj.get(QualifiersAttr).as[Option[ju.List[ConfigObject]]].getOrElse(ju.Collections.emptyList).asScala.foreach {
      obj =>
  ...
  private def readConditionals(config: Config): Config = {
    if !config.hasPath(Conditionals) then config
    else
      config.getList(Conditionals).asScala.foldLeft(config.withoutPath(Conditionals)) {
        (currentConfig, conditionalObject) =>
          ...
          if props(Condition).as[Boolean] then readConditionals(props(Config).as[Config]).withFallback(currentConfig)
          else currentConfig
      }
  }
  ...
  def loadBeanDefinitions(resourceConfig: Config): Int = {
    ...
    val beans = if config.hasPath(Beans) then config.getObject(Beans) else ConfigFactory.empty.root
    val aliases = if config.hasPath(Aliases) then config.getObject(Aliases) else ConfigFactory.empty.root
