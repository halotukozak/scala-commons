...
/**
 * Type-member version of [[MongoEntity]]. Used in type bounds where wildcards would wreak havoc (if `<: MongoEntity[_]`
 * was used instead of `<: BaseMongoEntity`).
 */
 ...
 *
 * Auto-ID entities are useful when there is no obvious choice for an unique and immutable `_id` and usually there is no
 * need to access it directly (e.g because queries are based on other fields).
 *
 * @tparam ID
 *   type of the (auto-generated) ID - it MUST be either raw [[org.bson.types.ObjectId]] or a _transparent wrapper_ over
 *   `ObjectId`, e.g. via [[ObjectIdWrapperCompanion]].
 */
