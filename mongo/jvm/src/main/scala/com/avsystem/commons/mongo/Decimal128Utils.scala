...
object Decimal128Utils {
  ...
  def fromBigDecimal(value: BigDecimal): Opt[Decimal128] =
    ...
    catch case Decimal128FormatException => Opt.Empty
  ...
  private def fromBigDecimal(initialValue: JBigDecimal, isNegative: Boolean): Decimal128 = {
    ...
    if exponent < MinExponent || exponent > MaxExponent || value.unscaledValue.bitLength > MaxBitLength then
    ...
    var i = 0
    while i < Math.min(64, bitLength) do {
      if significand.testBit(i) then {
        localLow |= 1L << i
      }
      i += 1
    }
    ...
    if value.signum == -1 || isNegative then {
  ...
  private def clampAndRound(initialValue: JBigDecimal): JBigDecimal = {
    ...
    if -initialValue.scale > MaxExponent then {
      val diff = -initialValue.scale - MaxExponent
      if initialValue.unscaledValue == BigIntZero then {
        value = new JBigDecimal(initialValue.unscaledValue, -MaxExponent)
      } else if diff + initialValue.precision > 34 then {
        throw Decimal128FormatException
      } else {
    ...
    } else if -initialValue.scale < MinExponent then {
      ...
      val divisor =
        if undiscardedPrecision == 0 then BigIntOne else BigIntTen.pow(diff)
      ...
      if extraPrecision > 0 then { // Again, only round exactly
  ...
  private def ensureExactRounding(initialValue: JBigDecimal, extraPrecision: Int): Int = {
    ...
    while i < significand.length do {
      if significand.charAt(i) != '0' then {
        throw Decimal128FormatException
      }
      i += 1
    }
