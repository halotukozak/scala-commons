...
sealed trait MongoRef[E, T] extends MongoProjection[E, T] with DataRefDsl[E, T] { self =>
  ...
  /**
   * Composes this reference with another one, effectively prepending a "prefix" to this reference. This is conceptually
   * similar to composing functions using `scala.Function1.compose`.
   */
 ...
 * subtype of the document, this implies an additional filter so that only a subset of documents matching the subtype is
 * returned.
 *
 * @tparam E
 *   the document type
...
sealed trait MongoPropertyRef[E, T]
   ...
   * method accepts a lambda simply for syntactic convenience - the "creator" gives you all the API for creating filters
   * on the value type which is usually shorter than creating them manually.
   *
   * {{{
   ...
   * be satisfied). Note that every operator may be used only once and this is not validated statically (a runtime error
   * is thrown when some operator is duplicated).
   *
   * {{{
   *   case class MyEntity(id: String, number: Int) extends MongoEntity[MyEntity]
   *   object MyEntity extends MongoEntityCompanion[MyEntity]
   ...
   */
  def satisfiesOperators(
    operators: MongoQueryOperator.Creator[T] => Seq[MongoQueryOperator[T]],
  ...
  @tailrec private def computePath[T0](
    ...
    case FieldRef(_: MongoToplevelRef[?, ?], fieldName, _, _) =>
      KeyEscaper.escape(fieldName) :: acc

    case FieldRef(prefix: MongoPropertyRef[E, ?], fieldName, _, _) =>
      computePath(onlyUpToArray, prefix, KeyEscaper.escape(fieldName) :: acc)

    case ArrayIndexRef(prefix, index, _) =>
      val newAcc = if onlyUpToArray then Nil else index.toString :: acc
  ...
  private def extractBson(doc: BsonDocument): BsonValue = this match {
    case FieldRef(_: MongoToplevelRef[?, ?], fieldName, _, fallback) =>
      doc.get(KeyEscaper.escape(fieldName)).opt.orElse(fallback).getOrElse(notFound)

    case FieldRef(prefix: MongoPropertyRef[E, ?], fieldName, _, fallback) =>
    ...
    case ArrayIndexRef(prefix, index, _) =>
      ...
      if index < array.size then array.get(index) else notFound
...
object MongoPropertyRef {
  ...
  implicit def optionalRefOps[E, O, T](ref: MongoPropertyRef[E, O])(implicit
  ...
  implicit def transparentRefOps[E, T, R](ref: MongoPropertyRef[E, T])(implicit
...
object MongoRef {
  ...
  final case class RootRef[T](format: MongoAdtFormat[T]) extends MongoToplevelRef[T, T] {
