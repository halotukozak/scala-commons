...
object MongoUpdate {
  ...
  final case class OperatorUpdate[T](operator: MongoUpdateOperator[T]) extends MongoUpdate[T]

  final case class MultiUpdate[E](propUpdates: Vector[PropertyUpdate[E, _]]) extends MongoDocumentUpdate[E]

  final case class PropertyUpdate[E, T](property: MongoPropertyRef[E, T], update: MongoUpdate[T])
...
sealed trait MongoDocumentUpdate[E] extends MongoUpdate[E] {
  ...
  def and(other: MongoDocumentUpdate[E]): MongoDocumentUpdate[E] = (this, other) match {
    case (MultiUpdate(propUpdates), propUpdate: PropertyUpdate[E, ?]) =>
      MultiUpdate(propUpdates :+ propUpdate)
    case (propUpdate: PropertyUpdate[E, ?], MultiUpdate(propUpdates)) =>
      MultiUpdate(propUpdate +: propUpdates)
    case (propUpdate1: PropertyUpdate[E, ?], propUpdate2: PropertyUpdate[E, ?]) =>
