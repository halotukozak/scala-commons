...
trait DataRefDsl[E, T] {
   ...
   * }}}
   *
   * When `T` is an `Option`, `Opt`, or similar `Option`-like type, the function may refer its `.get` method to return a
   ...
   * }}}
   *
   * You can use such "narrowed" reference as a prefix for accessing [[MongoPropertyRef]]s using the [[ref]] macro, e.g.
   * `thirdCaseRef.ref(_.data)`. You can also use it as a [[MongoProjection]] passed to one of [[TypedMongoCollection]]
   * methods. Note that in such case the projection also serves as a filter, limiting the results of the query only to
   * selected cases.
   */
  @explicitGenerics
