...
object TypedMongoClient {
  ...
  def apply(connectionString: ConnectionString, driverInformation: MongoDriverInformation): TypedMongoClient =
    new TypedMongoClient(MongoClients.create(connectionString, driverInformation))

  def apply(settings: MongoClientSettings, driverInformation: MongoDriverInformation): TypedMongoClient =
 ...
 */
class TypedMongoClient(val nativeClient: MongoClient, val clientSession: OptArg[TypedClientSession] = OptArg.Empty)
  ...
  def startSession(options: ClientSessionOptions = ClientSessionOptions.builder().build()): Task[TypedClientSession] =
   ...
   * objects.
   */
  def inSession[T](options: ClientSessionOptions = ClientSessionOptions.builder().build())(
    task: TypedClientSession => Task[T],
  ): Task[T] =
    startSession(options).bracket(task)(s => Task(s.close()))

  /**
   * Executes some code in context of a MongoDB client session, within a transaction. After the [[Task]] finishes, fails
   * or is cancelled, the transaction is either committed or aborted depending on the outcome and the session is closed.
   *
   * Note: in order for actual MongoDB operations to be associated with the session and the transaction, you need to use
   * `withSession` on [[TypedMongoClient]], [[TypedMongoDatabase]] or [[TypedMongoCollection]] and use the returned copy
   * of these objects.
   */
  def inTransaction[T](
  ...
  )(task: TypedClientSession => Task[T]): Task[T] =
