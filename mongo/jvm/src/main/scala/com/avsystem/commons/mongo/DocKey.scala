...
object DocKey {
  implicit def docKeySorting[T](docKey: DocKey[T, ? <: BsonValue]): DocKeySorting[T] = new DocKeySorting(docKey)
  implicit def docKeyFiltering[T](docKey: DocKey[T, ? <: BsonValue]): DocKeyFiltering[T] = new DocKeyFiltering(docKey)
}
