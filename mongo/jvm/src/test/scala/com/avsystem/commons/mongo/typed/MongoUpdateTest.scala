...
class MongoUpdateTest extends AnyFunSuite {
  ...
  private def toString(update: MongoDocumentUpdate[?]): String = {
  ...
  test("simple operators") {
    assert(
      toString(Rte.ref(_.int).set(5)) ==
        """{"$set": {"int": 5}}, []""",
    )
    assert(
      toString(Rte.ref(_.int).setOnInsert(5)) ==
        """{"$setOnInsert": {"int": 5}}, []""",
    )
    assert(
      toString(Rte.ref(_.int).inc(5)) ==
        """{"$inc": {"int": 5}}, []""",
    )
    assert(
      toString(Rte.ref(_.int).min(5)) ==
        """{"$min": {"int": 5}}, []""",
    )
    assert(
      toString(Rte.ref(_.int).max(5)) ==
        """{"$max": {"int": 5}}, []""",
    )
    assert(
      toString(Rte.ref(_.int).mul(5)) ==
        """{"$mul": {"int": 5}}, []""",
    )
    assert(
      toString(Rte.ref(_.tstamp).currentDate) ==
  ...
  test("collection operators") {
    assert(
      toString(Rte.ref(_.intList).push(1, 2, 3)) ==
    ...
    assert(
      toString(Rte.ref(_.intList).addToSet(1, 2, 3)) ==
        """{"$addToSet": {"intList": {"$each": [1, 2, 3]}}}, []""",
    )
    assert(
      toString(Rte.ref(_.intList).addToSet(Seq(1, 2, 3))) ==
        """{"$addToSet": {"intList": {"$each": [1, 2, 3]}}}, []""",
    )
    assert(
      toString(Rte.ref(_.intList).popFirst) ==
        """{"$pop": {"intList": -1}}, []""",
    )
    assert(
      toString(Rte.ref(_.intList).popLast) ==
        """{"$pop": {"intList": 1}}, []""",
    )
    assert(
      toString(Rte.ref(_.intList).pull(_ < 5)) ==
        """{"$pull": {"intList": {"$lt": 5}}}, []""",
    )
    assert(
    ...
    assert(
      toString(Rte.ref(_.intList).pullAll(1, 2, 3)) ==
        """{"$pullAll": {"intList": [1, 2, 3]}}, []""",
    )
    assert(
      toString(Rte.ref(_.intList).pullAll(Seq(1, 2, 3))) ==
        """{"$pullAll": {"intList": [1, 2, 3]}}, []""",
    )
    assert(
      toString(Rte.ref(_.intList).updateFirstMatching(_.set(5))) ==
        """{"$set": {"intList.$": 5}}, []""",
    )
    assert(
      toString(Rte.ref(_.intList).updateAll(_.set(5))) ==
        """{"$set": {"intList.$[]": 5}}, []""",
    )
    assert(
    ...
    assert(
      toString(Rte.ref(_.innerList).updateAll(_.ref(_.int).set(5))) ==
        """{"$set": {"innerList.$[].int": 5}}, []""",
    )
    assert(
