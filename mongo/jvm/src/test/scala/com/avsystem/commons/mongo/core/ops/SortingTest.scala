...
class SortingTest extends AnyFunSuite {
  ...
  test("docKey tests") {
    ...
    assert(
      orderBy(someDocKey.ascending, otherDocKey.descending) === Sorts
        .orderBy(Sorts.ascending(someKey), Sorts.descending(otherKey)),
    )
  }
  ...
  test("bsonRef tests") {
    ...
    assert(
      orderBy(someRef.ascending, otherRef.descending) === Sorts
        .orderBy(Sorts.ascending(someKey), Sorts.descending(otherKey)),
    )
  }
}
